...
func GetKVMTable(resource runtime.Object) *metav1.Table {
	...
	for _, clusterList := range clusterLists {
		...
		case *cluster.V4ClusterList:
			table.Rows = append(table.Rows, getKVMV4ClusterListRow(c))
...
func getKVMV4ClusterListRow(res *cluster.V4ClusterList) metav1.TableRow {
	clusterConfig := res.Items[0].(*corev1alpha1.KVMClusterConfig)
	config := res.Items[1].(*providerv1alpha1.KVMConfig)
