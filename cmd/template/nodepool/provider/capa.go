...
func WriteCAPATemplate(out io.Writer, config NodePoolCRsConfig) error {
	...
	nodepoolTemplate, err := getCAPANodepoolTemplate(config, "machinepool")
	if err != nil {
		return err
	}
...
func getCAPANodepoolTemplate(config NodePoolCRsConfig, flavor string) (client.Template, error) {
	var err error
