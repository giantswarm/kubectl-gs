...
import (
	...
	"github.com/giantswarm/kubectl-gs/v6/pkg/graphql"
)
...
func getInstallationInfo(ctx context.Context, gqlClient graphql.Client) (installationInfo, error) {
	...
	if err != nil {
		return installationInfo{}, microerror.Maskf(cannotGetInstallationInfoError, "make sure you're connected to the internet, OIDC authentication is configured for the Management API, and the Athena service is up and running\n%s", err.Error())
	}
